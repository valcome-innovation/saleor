...
import { createAttribute, createAttributeProductLinks, createAttributeValue, createProduct } from './factories';
import { ticketProductType, ticketsCategory } from './static-objects';

const ticketTypeAttribute = createAttribute(
  { name: 'Ticket Type', slug: 'ticket-type', input_type: 'dropdown', isRequired: true }
);
const leaguesAttribute = createAttribute(
  { name: 'Leagues', slug: 'leagues', input_type: 'multiselect', isRequired: false }
);
const teamsAttribute = createAttribute(
  { name: 'Teams', slug: 'teams', input_type: 'multiselect', isRequired: false }
);
...
const attributeLinks = [
  ...
  leaguesAttribute,
  teamsAttribute
].map(attribute => createAttributeProductLinks(attribute)).flat();
...
const singleProducts = teamValues.map(team => createProduct({
  name: 'Single',
...
const seasonProducts = teamValues.map(team => createProduct({
  name: 'Season',
  postfix: `- ${team.fields.name}`,
  postfixSlug: `-${team.fields.slug}`,
  price: '199.000',
...
seasonProducts.push(...createProduct({
  name: 'Season',
  postfix: '',
  postfixSlug: '',
  price: '299.000',
  attributes: [{ attribute: ticketTypeAttribute, values: [seasonAttribute] }]
}));

const dayProducts = leagueValues.map(league => createProduct({
  name: 'Day',
...
const monthProducts = leagueValues.map(league => {
  const products = teamValues.map(team => createProduct({
      name: 'Month',
